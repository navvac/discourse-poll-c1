...
createWidget("discourse-poll-pie-canvas", {
  ...
  init(attrs) {
    loadScript("/javascripts/Chart.min.js").then(() => {
      ...
      const cooked_labels = attrs.poll.options.mapBy("cooked_html");
      const config = pieChartConfig(data, labels, cooked_labels);

      const el = document.getElementById(`poll-results-chart-${attrs.id}`);
      // eslint-disable-next-line
      let chart = new Chart(el.getContext("2d"), config);
      let legendsContainer = document.getElementById(
        `poll-results-legend-${attrs.id}`
      );
      legendsContainer.innerHTML = chart.generateLegend();
      let scrollContainer = legendsContainer.parentElement;
      // this.appEvents.trigger("post-stream:refresh");
      // next(() => lightbox(this.element.parent.parent.parent.parent, this.siteSettings));
      const masterPostContainer = document.getElementById(`post_${attrs.post.post_number}`);

      el.onclick = function(evt) {
        let activePoints = chart.getElementsAtEvent(evt);
        if (activePoints[0]) {
          const optionIndex = activePoints[0]._index;
          let targetLegendTopPos = legendsContainer.children.item(optionIndex).offsetTop;
          // scrollContainer.scrollTop = targetLegendTopPos;
          scrollTo(scrollContainer, targetLegendTopPos, 0.7)
          // console.log(activePoints);

        }
      }
      next(() => lightbox(masterPostContainer, this.siteSettings));
    });
  },
...
function pieChartConfig(data, labels, cooked_labels, opts = {}) {
      ...
      legendCallback: function (chart) {
        ...
        for (let i = 0; i < labels.length; i++) {
          legends += `<div class="legend"><span class="swatch" style="background-color:
            ${chart.data.datasets[0].backgroundColor[i]}"></span>${cooked_labels[i]}</div>`;
        }
        return legends;
      },
    },
