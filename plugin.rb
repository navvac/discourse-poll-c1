      ...
      def create!(post_id, poll)
        ...
        poll["options"].each do |option|
          PollOption.create!(
            ...
            cooked_html: cook_option_html(option["html"].presence&.strip, post)
          )
        end
      end

      def cook_option_html(html_in, post_in)
        doc = Nokogiri::HTML5::fragment(html_in)
        extracted_images =
            doc.css("img[src]") -
            # minus data images
            ...
            # minus emojis
            doc.css("img.emoji")
        cpp = CookedPostProcessor.new(post_in)
        extracted_images.each do |img|
          cpp.convert_to_link!(img)
        end
        doc.try(:to_html)
      end
